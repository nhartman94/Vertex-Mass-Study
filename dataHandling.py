'''
Module for useful data handling functions that I use across my notebooks
'''
import numpy as np
...
def flatten_trks(column):
    '''
    Args:
    -----
        column: a column of a pandas df whose entries are lists of lists 
        (Note: I'm not sure if this would work if the entries were lists)
                e.g.: my_df['some_variable'] 
    Returns:
    --------    
        flattened out version of the column. 
        
    Modified from Michela's flatten function above
    '''
    try:
        return np.array([t for e in column for v in e for t in v])
    except (TypeError, ValueError):
        return column
